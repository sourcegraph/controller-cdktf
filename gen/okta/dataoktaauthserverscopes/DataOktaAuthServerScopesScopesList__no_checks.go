//go:build no_runtime_type_checking

package dataoktaauthserverscopes

// Building without runtime type checking enabled, so all the below just return nil

func (d *jsiiProxy_DataOktaAuthServerScopesScopesList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
	return nil
}

func (d *jsiiProxy_DataOktaAuthServerScopesScopesList) validateGetParameters(index *float64) error {
	return nil
}

func (d *jsiiProxy_DataOktaAuthServerScopesScopesList) validateResolveParameters(_context cdktf.IResolveContext) error {
	return nil
}

func (j *jsiiProxy_DataOktaAuthServerScopesScopesList) validateSetTerraformAttributeParameters(val *string) error {
	return nil
}

func (j *jsiiProxy_DataOktaAuthServerScopesScopesList) validateSetTerraformResourceParameters(val cdktf.IInterpolatingParent) error {
	return nil
}

func (j *jsiiProxy_DataOktaAuthServerScopesScopesList) validateSetWrapsSetParameters(val *bool) error {
	return nil
}

func validateNewDataOktaAuthServerScopesScopesListParameters(terraformResource cdktf.IInterpolatingParent, terraformAttribute *string, wrapsSet *bool) error {
	return nil
}

