...
func (c *jsiiProxy_ClusterRoleBindingSubjectList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_ClusterRoleBindingSubjectList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*ClusterRoleBindingSubject:
		val := val.(*[]*ClusterRoleBindingSubject)
	...
	case []*ClusterRoleBindingSubject:
		val_ := val.([]*ClusterRoleBindingSubject)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*ClusterRoleBindingSubject; received %#v (a %T)", val, val)
		}
	}
