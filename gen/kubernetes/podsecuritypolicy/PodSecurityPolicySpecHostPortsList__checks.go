...
func (p *jsiiProxy_PodSecurityPolicySpecHostPortsList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_PodSecurityPolicySpecHostPortsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*PodSecurityPolicySpecHostPorts:
		val := val.(*[]*PodSecurityPolicySpecHostPorts)
	...
	case []*PodSecurityPolicySpecHostPorts:
		val_ := val.([]*PodSecurityPolicySpecHostPorts)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*PodSecurityPolicySpecHostPorts; received %#v (a %T)", val, val)
		}
	}
