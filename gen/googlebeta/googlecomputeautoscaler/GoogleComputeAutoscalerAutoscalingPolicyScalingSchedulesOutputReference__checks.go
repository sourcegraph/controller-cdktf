...
func (j *jsiiProxy_GoogleComputeAutoscalerAutoscalingPolicyScalingSchedulesOutputReference) validateSetInternalValueParameters(val interface{}) error {
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: *GoogleComputeAutoscalerAutoscalingPolicyScalingSchedules, cdktf.IResolvable; received %#v (a %T)", val, val)
		}
	}
