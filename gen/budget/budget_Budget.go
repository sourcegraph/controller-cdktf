...
func NewBudget(scope constructs.Construct, id *string, options *BudgetOptions) Budget {
	_init_.Initialize()

	if err := validateNewBudgetParameters(scope, id, options); err != nil {
		panic(err)
	}
	j := jsiiProxy_Budget{}
...
func NewBudget_Override(b Budget, scope constructs.Construct, id *string, options *BudgetOptions) {
	_init_.Initialize()
