...
func (m *jsiiProxy_MedialiveInputSourcesList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_MedialiveInputSourcesList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*MedialiveInputSources:
		val := val.(*[]*MedialiveInputSources)
	...
	case []*MedialiveInputSources:
		val_ := val.([]*MedialiveInputSources)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*MedialiveInputSources; received %#v (a %T)", val, val)
		}
	}
