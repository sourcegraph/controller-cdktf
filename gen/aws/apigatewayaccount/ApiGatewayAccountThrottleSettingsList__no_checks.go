//go:build no_runtime_type_checking

package apigatewayaccount

// Building without runtime type checking enabled, so all the below just return nil

func (a *jsiiProxy_ApiGatewayAccountThrottleSettingsList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
	return nil
}

func (a *jsiiProxy_ApiGatewayAccountThrottleSettingsList) validateGetParameters(index *float64) error {
	return nil
}

func (a *jsiiProxy_ApiGatewayAccountThrottleSettingsList) validateResolveParameters(_context cdktf.IResolveContext) error {
	return nil
}

func (j *jsiiProxy_ApiGatewayAccountThrottleSettingsList) validateSetTerraformAttributeParameters(val *string) error {
	return nil
}

func (j *jsiiProxy_ApiGatewayAccountThrottleSettingsList) validateSetTerraformResourceParameters(val cdktf.IInterpolatingParent) error {
	return nil
}

func (j *jsiiProxy_ApiGatewayAccountThrottleSettingsList) validateSetWrapsSetParameters(val *bool) error {
	return nil
}

func validateNewApiGatewayAccountThrottleSettingsListParameters(terraformResource cdktf.IInterpolatingParent, terraformAttribute *string, wrapsSet *bool) error {
	return nil
}

