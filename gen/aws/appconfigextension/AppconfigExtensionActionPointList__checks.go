...
func (a *jsiiProxy_AppconfigExtensionActionPointList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_AppconfigExtensionActionPointList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AppconfigExtensionActionPoint:
		val := val.(*[]*AppconfigExtensionActionPoint)
	...
	case []*AppconfigExtensionActionPoint:
		val_ := val.([]*AppconfigExtensionActionPoint)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*AppconfigExtensionActionPoint; received %#v (a %T)", val, val)
		}
	}
