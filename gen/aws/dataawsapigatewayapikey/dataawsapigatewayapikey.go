...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsapigatewayapikey/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/api_gateway_api_key aws_api_gateway_api_key}.
...
func NewDataAwsApiGatewayApiKey(scope constructs.Construct, id *string, config *DataAwsApiGatewayApiKeyConfig) DataAwsApiGatewayApiKey {
	...
	if err := validateNewDataAwsApiGatewayApiKeyParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsApiGatewayApiKey_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsApiGatewayApiKey_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
