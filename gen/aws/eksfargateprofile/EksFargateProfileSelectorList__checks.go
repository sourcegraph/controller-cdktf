...
func (e *jsiiProxy_EksFargateProfileSelectorList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_EksFargateProfileSelectorList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*EksFargateProfileSelector:
		val := val.(*[]*EksFargateProfileSelector)
	...
	case []*EksFargateProfileSelector:
		val_ := val.([]*EksFargateProfileSelector)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*EksFargateProfileSelector; received %#v (a %T)", val, val)
		}
	}
