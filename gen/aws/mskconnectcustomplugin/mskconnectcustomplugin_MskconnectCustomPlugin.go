...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/mskconnectcustomplugin/internal"
)
...
func NewMskconnectCustomPlugin(scope constructs.Construct, id *string, config *MskconnectCustomPluginConfig) MskconnectCustomPlugin {
	...
	if err := validateNewMskconnectCustomPluginParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func (m *jsiiProxy_MskconnectCustomPlugin) PutLocation(value *MskconnectCustomPluginLocation) {
	if err := m.validatePutLocationParameters(value); err != nil {
		panic(err)
	}
	_jsii_.InvokeVoid(
		m,
