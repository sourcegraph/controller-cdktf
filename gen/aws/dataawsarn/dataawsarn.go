...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsarn/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/arn aws_arn}.
...
func NewDataAwsArn(scope constructs.Construct, id *string, config *DataAwsArnConfig) DataAwsArn {
	...
	if err := validateNewDataAwsArnParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsArn_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsArn_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
