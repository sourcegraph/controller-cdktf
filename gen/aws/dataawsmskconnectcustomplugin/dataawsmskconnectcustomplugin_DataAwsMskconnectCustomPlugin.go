...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsmskconnectcustomplugin/internal"
)
...
func NewDataAwsMskconnectCustomPlugin(scope constructs.Construct, id *string, config *DataAwsMskconnectCustomPluginConfig) DataAwsMskconnectCustomPlugin {
	...
	if err := validateNewDataAwsMskconnectCustomPluginParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsMskconnectCustomPlugin_IsConstruct(x interface{}) *bool {
	...
	if err := validateDataAwsMskconnectCustomPlugin_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
