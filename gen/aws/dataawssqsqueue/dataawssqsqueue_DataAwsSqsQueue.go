...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawssqsqueue/internal"
)
...
func NewDataAwsSqsQueue(scope constructs.Construct, id *string, config *DataAwsSqsQueueConfig) DataAwsSqsQueue {
	...
	if err := validateNewDataAwsSqsQueueParameters(scope, id, config); err != nil {
		panic(err)
	}
	j := jsiiProxy_DataAwsSqsQueue{}

	_jsii_.Create(
		"aws.dataAwsSqsQueue.DataAwsSqsQueue",
...
func DataAwsSqsQueue_IsConstruct(x interface{}) *bool {
	...
	if err := validateDataAwsSqsQueue_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
