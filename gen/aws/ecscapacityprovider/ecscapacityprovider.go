...
// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/resources/ecs_capacity_provider aws_ecs_capacity_provider}.
...
func NewEcsCapacityProvider(scope constructs.Construct, id *string, config *EcsCapacityProviderConfig) EcsCapacityProvider {
	...
	if err := validateNewEcsCapacityProviderParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func EcsCapacityProvider_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateEcsCapacityProvider_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
...
func (e *jsiiProxy_EcsCapacityProvider) AddMoveTarget(moveTarget *string) {
...
func (e *jsiiProxy_EcsCapacityProvider) PutAutoScalingGroupProvider(value *EcsCapacityProviderAutoScalingGroupProvider) {
	if err := e.validatePutAutoScalingGroupProviderParameters(value); err != nil {
		panic(err)
	}
	_jsii_.InvokeVoid(
		e,
