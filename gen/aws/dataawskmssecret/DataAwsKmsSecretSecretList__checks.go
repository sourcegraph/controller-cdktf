...
func (j *jsiiProxy_DataAwsKmsSecretSecretList) validateSetInternalValueParameters(val interface{}) error {
	switch val.(type) {
	case cdktf.IResolvable:
		// ok
	case *[]*DataAwsKmsSecretSecret:
		val := val.(*[]*DataAwsKmsSecretSecret)
	...
	case []*DataAwsKmsSecretSecret:
		val_ := val.([]*DataAwsKmsSecretSecret)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*DataAwsKmsSecretSecret; received %#v (a %T)", val, val)
		}
	}
...
func (j *jsiiProxy_DataAwsKmsSecretSecretList) validateSetTerraformAttributeParameters(val *string) error {
	if val == nil {
...
func (j *jsiiProxy_DataAwsKmsSecretSecretList) validateSetWrapsSetParameters(val *bool) error {
	if val == nil {
		return fmt.Errorf("parameter val is required, but nil was provided")
	}
