...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/guarddutythreatintelset/internal"
)
...
func NewGuarddutyThreatintelset(scope constructs.Construct, id *string, config *GuarddutyThreatintelsetConfig) GuarddutyThreatintelset {
	...
	if err := validateNewGuarddutyThreatintelsetParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func GuarddutyThreatintelset_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateGuarddutyThreatintelset_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
...
func (g *jsiiProxy_GuarddutyThreatintelset) AddMoveTarget(moveTarget *string) {
