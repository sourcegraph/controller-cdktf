...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsmskkafkaversion/internal"
)
...
func NewDataAwsMskKafkaVersion(scope constructs.Construct, id *string, config *DataAwsMskKafkaVersionConfig) DataAwsMskKafkaVersion {
	...
	if err := validateNewDataAwsMskKafkaVersionParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsMskKafkaVersion_IsConstruct(x interface{}) *bool {
	...
	if err := validateDataAwsMskKafkaVersion_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
