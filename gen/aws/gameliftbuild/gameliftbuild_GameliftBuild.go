...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/gameliftbuild/internal"
)
...
func NewGameliftBuild(scope constructs.Construct, id *string, config *GameliftBuildConfig) GameliftBuild {
	...
	if err := validateNewGameliftBuildParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func GameliftBuild_IsConstruct(x interface{}) *bool {
	...
	if err := validateGameliftBuild_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
func (g *jsiiProxy_GameliftBuild) PutStorageLocation(value *GameliftBuildStorageLocation) {
	if err := g.validatePutStorageLocationParameters(value); err != nil {
		panic(err)
	}
	_jsii_.InvokeVoid(
		g,
