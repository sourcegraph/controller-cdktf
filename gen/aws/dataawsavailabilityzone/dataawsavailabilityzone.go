...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsavailabilityzone/internal"
)
...
func NewDataAwsAvailabilityZone(scope constructs.Construct, id *string, config *DataAwsAvailabilityZoneConfig) DataAwsAvailabilityZone {
	...
	if err := validateNewDataAwsAvailabilityZoneParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsAvailabilityZone_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsAvailabilityZone_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
