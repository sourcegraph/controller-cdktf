package timestreamwritedatabase
...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/timestreamwritedatabase/internal"
)
...
func NewTimestreamwriteDatabase(scope constructs.Construct, id *string, config *TimestreamwriteDatabaseConfig) TimestreamwriteDatabase {
	...
	if err := validateNewTimestreamwriteDatabaseParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func TimestreamwriteDatabase_IsConstruct(x interface{}) *bool {
	...
	if err := validateTimestreamwriteDatabase_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
