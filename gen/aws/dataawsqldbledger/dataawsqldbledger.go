...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsqldbledger/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/qldb_ledger aws_qldb_ledger}.
...
func NewDataAwsQldbLedger(scope constructs.Construct, id *string, config *DataAwsQldbLedgerConfig) DataAwsQldbLedger {
	...
	if err := validateNewDataAwsQldbLedgerParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsQldbLedger_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsQldbLedger_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
