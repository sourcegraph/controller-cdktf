...
func (j *jsiiProxy_TransferUserHomeDirectoryMappingsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*TransferUserHomeDirectoryMappings:
		val := val.(*[]*TransferUserHomeDirectoryMappings)
	...
	case []*TransferUserHomeDirectoryMappings:
		val_ := val.([]*TransferUserHomeDirectoryMappings)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*TransferUserHomeDirectoryMappings; received %#v (a %T)", val, val)
		}
	}
