...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawscanonicaluserid/internal"
)
...
func NewDataAwsCanonicalUserId(scope constructs.Construct, id *string, config *DataAwsCanonicalUserIdConfig) DataAwsCanonicalUserId {
	...
	if err := validateNewDataAwsCanonicalUserIdParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsCanonicalUserId_IsConstruct(x interface{}) *bool {
	...
	if err := validateDataAwsCanonicalUserId_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
