...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/acmpcapolicy/internal"
)
...
func NewAcmpcaPolicy(scope constructs.Construct, id *string, config *AcmpcaPolicyConfig) AcmpcaPolicy {
	...
	if err := validateNewAcmpcaPolicyParameters(scope, id, config); err != nil {
		panic(err)
	}
	j := jsiiProxy_AcmpcaPolicy{}

	_jsii_.Create(
		"aws.acmpcaPolicy.AcmpcaPolicy",
...
func AcmpcaPolicy_IsConstruct(x interface{}) *bool {
	...
	if err := validateAcmpcaPolicy_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
