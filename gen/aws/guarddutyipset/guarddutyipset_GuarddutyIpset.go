...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/guarddutyipset/internal"
)
...
func NewGuarddutyIpset(scope constructs.Construct, id *string, config *GuarddutyIpsetConfig) GuarddutyIpset {
	...
	if err := validateNewGuarddutyIpsetParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func GuarddutyIpset_IsConstruct(x interface{}) *bool {
	...
	if err := validateGuarddutyIpset_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
