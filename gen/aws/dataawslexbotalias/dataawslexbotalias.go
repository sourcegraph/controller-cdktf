...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawslexbotalias/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/lex_bot_alias aws_lex_bot_alias}.
...
func NewDataAwsLexBotAlias(scope constructs.Construct, id *string, config *DataAwsLexBotAliasConfig) DataAwsLexBotAlias {
	...
	if err := validateNewDataAwsLexBotAliasParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsLexBotAlias_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsLexBotAlias_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
