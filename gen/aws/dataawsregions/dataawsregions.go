...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsregions/internal"
)
...
func NewDataAwsRegions(scope constructs.Construct, id *string, config *DataAwsRegionsConfig) DataAwsRegions {
	...
	if err := validateNewDataAwsRegionsParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsRegions_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsRegions_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
