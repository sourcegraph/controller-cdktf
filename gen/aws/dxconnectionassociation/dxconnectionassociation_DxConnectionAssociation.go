...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dxconnectionassociation/internal"
)
...
func NewDxConnectionAssociation(scope constructs.Construct, id *string, config *DxConnectionAssociationConfig) DxConnectionAssociation {
	...
	if err := validateNewDxConnectionAssociationParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DxConnectionAssociation_IsConstruct(x interface{}) *bool {
	...
	if err := validateDxConnectionAssociation_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
