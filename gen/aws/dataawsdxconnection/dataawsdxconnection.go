...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsdxconnection/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/dx_connection aws_dx_connection}.
...
func NewDataAwsDxConnection(scope constructs.Construct, id *string, config *DataAwsDxConnectionConfig) DataAwsDxConnection {
	...
	if err := validateNewDataAwsDxConnectionParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsDxConnection_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsDxConnection_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
