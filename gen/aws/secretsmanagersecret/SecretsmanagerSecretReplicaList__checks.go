...
func (j *jsiiProxy_SecretsmanagerSecretReplicaList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*SecretsmanagerSecretReplica:
		val := val.(*[]*SecretsmanagerSecretReplica)
	...
	case []*SecretsmanagerSecretReplica:
		val_ := val.([]*SecretsmanagerSecretReplica)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*SecretsmanagerSecretReplica; received %#v (a %T)", val, val)
		}
	}
