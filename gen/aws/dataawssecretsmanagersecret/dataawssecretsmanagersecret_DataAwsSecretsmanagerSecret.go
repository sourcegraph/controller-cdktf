...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawssecretsmanagersecret/internal"
)
...
func NewDataAwsSecretsmanagerSecret(scope constructs.Construct, id *string, config *DataAwsSecretsmanagerSecretConfig) DataAwsSecretsmanagerSecret {
	...
	if err := validateNewDataAwsSecretsmanagerSecretParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsSecretsmanagerSecret_IsConstruct(x interface{}) *bool {
	...
	if err := validateDataAwsSecretsmanagerSecret_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
