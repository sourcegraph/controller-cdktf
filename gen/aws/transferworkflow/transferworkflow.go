...
// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/resources/transfer_workflow aws_transfer_workflow}.
...
func NewTransferWorkflow(scope constructs.Construct, id *string, config *TransferWorkflowConfig) TransferWorkflow {
	...
	if err := validateNewTransferWorkflowParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func TransferWorkflow_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateTransferWorkflow_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
...
func (t *jsiiProxy_TransferWorkflow) AddMoveTarget(moveTarget *string) {
