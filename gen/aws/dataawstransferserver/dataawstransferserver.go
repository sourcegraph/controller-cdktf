...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawstransferserver/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/transfer_server aws_transfer_server}.
...
func NewDataAwsTransferServer(scope constructs.Construct, id *string, config *DataAwsTransferServerConfig) DataAwsTransferServer {
	...
	if err := validateNewDataAwsTransferServerParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsTransferServer_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsTransferServer_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
