...
// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/ssm_patch_baseline aws_ssm_patch_baseline}.
...
func NewDataAwsSsmPatchBaseline(scope constructs.Construct, id *string, config *DataAwsSsmPatchBaselineConfig) DataAwsSsmPatchBaseline {
	...
	if err := validateNewDataAwsSsmPatchBaselineParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsSsmPatchBaseline_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsSsmPatchBaseline_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
