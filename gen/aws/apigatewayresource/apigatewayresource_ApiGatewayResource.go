...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/apigatewayresource/internal"
)
...
func NewApiGatewayResource(scope constructs.Construct, id *string, config *ApiGatewayResourceConfig) ApiGatewayResource {
	...
	if err := validateNewApiGatewayResourceParameters(scope, id, config); err != nil {
		panic(err)
	}
	j := jsiiProxy_ApiGatewayResource{}

	_jsii_.Create(
		"aws.apiGatewayResource.ApiGatewayResource",
...
func (j *jsiiProxy_ApiGatewayResource)SetPathPart(val *string) {
	if err := j.validateSetPathPartParameters(val); err != nil {
		panic(err)
	}
	_jsii_.Set(
		j,
...
func ApiGatewayResource_IsConstruct(x interface{}) *bool {
	...
	if err := validateApiGatewayResource_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
