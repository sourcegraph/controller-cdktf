...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsipranges/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/ip_ranges aws_ip_ranges}.
...
func NewDataAwsIpRanges(scope constructs.Construct, id *string, config *DataAwsIpRangesConfig) DataAwsIpRanges {
	...
	if err := validateNewDataAwsIpRangesParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsIpRanges_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsIpRanges_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
