...
func (s *jsiiProxy_SesReceiptRuleBounceActionList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_SesReceiptRuleBounceActionList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*SesReceiptRuleBounceAction:
		val := val.(*[]*SesReceiptRuleBounceAction)
	...
	case []*SesReceiptRuleBounceAction:
		val_ := val.([]*SesReceiptRuleBounceAction)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*SesReceiptRuleBounceAction; received %#v (a %T)", val, val)
		}
	}
