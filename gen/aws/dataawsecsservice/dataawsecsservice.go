...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawsecsservice/internal"
)

// Represents a {@link https://registry.terraform.io/providers/hashicorp/aws/4.54.0/docs/data-sources/ecs_service aws_ecs_service}.
...
func NewDataAwsEcsService(scope constructs.Construct, id *string, config *DataAwsEcsServiceConfig) DataAwsEcsService {
	...
	if err := validateNewDataAwsEcsServiceParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsEcsService_GenerateConfigForImport(scope constructs.Construct, importToId *string, importFromId *string, provider cdktf.TerraformProvider) cdktf.ImportableResource {
	_init_.Initialize()

	if err := validateDataAwsEcsService_GenerateConfigForImportParameters(scope, importToId, importFromId); err != nil {
		panic(err)
	}
