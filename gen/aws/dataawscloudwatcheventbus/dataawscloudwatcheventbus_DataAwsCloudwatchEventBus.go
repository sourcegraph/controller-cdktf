...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/dataawscloudwatcheventbus/internal"
)
...
func NewDataAwsCloudwatchEventBus(scope constructs.Construct, id *string, config *DataAwsCloudwatchEventBusConfig) DataAwsCloudwatchEventBus {
	...
	if err := validateNewDataAwsCloudwatchEventBusParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataAwsCloudwatchEventBus_IsConstruct(x interface{}) *bool {
	...
	if err := validateDataAwsCloudwatchEventBus_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
