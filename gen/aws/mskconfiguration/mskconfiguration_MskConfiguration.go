...
import (
	...
	"github.com/sourcegraph/controller-cdktf/gen/aws/mskconfiguration/internal"
)
...
func NewMskConfiguration(scope constructs.Construct, id *string, config *MskConfigurationConfig) MskConfiguration {
	...
	if err := validateNewMskConfigurationParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func (j *jsiiProxy_MskConfiguration)SetKafkaVersions(val *[]*string) {
	if err := j.validateSetKafkaVersionsParameters(val); err != nil {
		panic(err)
	}
	_jsii_.Set(
		j,
...
func (j *jsiiProxy_MskConfiguration)SetServerProperties(val *string) {
	if err := j.validateSetServerPropertiesParameters(val); err != nil {
		panic(err)
	}
	_jsii_.Set(
		j,
...
func MskConfiguration_IsConstruct(x interface{}) *bool {
	...
	if err := validateMskConfiguration_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
